from mdp_q.gen_mdp import gen_g
from mdp_q.sim_tools import sim_one_step
from mdp_q.optimization import vi, evi
import numpy as np
    ...
    if reward_type == "exploration":
        ...
        for s in S:
            for a in A[s]:
                ...
                p_hat_dict[(s, a)] = np.ones(len_S) / (len_S * 1.)
        def grad_g(w):
            return (1. / K) * (np.ones(K) + np.maximum(target - w, 0)) 
